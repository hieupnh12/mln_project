...
import { fetchLessonDetailApi, updateLessonApi } from '../api/course-structure.api';
...
import { TeacherMindmapCanvas } from '../components/teacher-mindmap-canvas';

export function TeacherLessonMindmapPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      await runWithAsyncActivity({
        label: 'Đang lưu',
        detail: 'Đang đồng bộ sơ đồ tư duy',
    ...
    <div className="flex h-screen bg-[var(--color-background)] overflow-hidden">
      ...
      <div className="relative h-full flex z-10 shrink-0">
        ...
        <button
          ...
          className="absolute right-full top-1/2 -translate-y-1/2 w-6 z-20 bg-white border-y border-l border-[var(--color-outline-variant)] rounded-l-lg py-4 shadow-[-4px_0_8px_rgba(0,0,0,0.02)] hover:bg-gray-50 transition-colors flex items-center justify-center translate-x-[1px]"
          ...
          <aside className="h-full flex flex-col p-4 overflow-y-auto">
            <div className="flex items-center gap-3 mb-4">
              <button
                ...
                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full hover:bg-black/5 transition"
                type="button"
                title="Quay lại"
              >
              ...
              <div className="min-w-0">
                <span className="block text-[10px] font-bold uppercase tracking-wider text-[var(--color-outline)] truncate">
                  {chapterName}
                </span>
                <h1 className="text-lg font-bold text-[var(--color-primary)] truncate">
                  {lessonTitle}
                </h1>
              </div>
            </div>

            <div className="mb-4 bg-[var(--color-surface-container-low)] p-3 rounded-xl border border-[var(--color-outline-variant)]">
              <h2 className="text-sm font-bold text-[var(--color-primary-container)] mb-1.5 flex items-center gap-2">
                <span className="material-symbols-outlined text-[16px] text-[var(--color-secondary)]">hub</span>
                Công cụ bóc tách AI
              </h2>
              <p className="text-[11px] text-[var(--color-on-surface-variant)] mb-3 leading-relaxed">
                Copy Prompt bên dưới dán vào AI để trích xuất sơ đồ chi tiết và dán ngược lại JSON vào hệ thống:
              </p>
              <button
                onClick={copyPromptText}
                className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-[var(--color-secondary-container)] text-[var(--color-on-secondary-container)] px-3 py-1.5 text-xs font-bold transition hover:opacity-90"
                type="button"
              >
            ...
            <div className="flex flex-col mb-4">
              <label className="block text-[11px] font-bold uppercase tracking-wider text-[var(--color-outline)] mb-1.5">
                Dữ liệu sơ đồ (JSON)
              </label>
              <textarea
                value={jsonText}
                onChange={(e) => setJsonText(e.target.value)}
                placeholder="Dán mã JSON..."
                className="h-[180px] w-full rounded-lg border border-[var(--color-outline-variant)] p-2.5 text-[11px] font-mono bg-[var(--color-surface-container-lowest)] focus:border-[var(--color-primary)] focus:ring-1 focus:ring-[var(--color-primary)] focus:outline-none resize-y transition-shadow shadow-inner text-[var(--color-on-surface)]"
              />
              <button
                onClick={handleApplyJson}
                className="mt-2 w-full inline-flex items-center justify-center gap-2 rounded-lg bg-[var(--color-primary-container)] text-[var(--color-on-primary)] px-4 py-2 text-xs font-bold transition hover:opacity-90"
                type="button"
              >
            ...
            <div className="mt-4 border-t border-[var(--color-outline-variant)] pt-3 flex flex-col gap-2.5">
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="w-full rounded-lg bg-[var(--color-primary)] py-2.5 text-sm font-bold text-white hover:opacity-90 transition disabled:opacity-50 text-center shadow-md flex items-center justify-center gap-2"
                type="button"
              >
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => window.history.back()}
                  className="flex-1 rounded-lg border border-[var(--color-outline-variant)] py-1.5 text-xs font-semibold text-[var(--color-on-surface-variant)] hover:bg-[var(--color-surface-container-high)] transition text-center flex items-center justify-center gap-1"
                  type="button"
                >
                ...
                <button
                  ...
                  className="flex-1 rounded-lg border border-[var(--color-outline-variant)] py-2 text-xs font-semibold text-[var(--color-on-surface-variant)] hover:bg-[var(--color-surface-container-high)] transition text-center disabled:opacity-50 flex items-center justify-center gap-1"
                  type="button"
