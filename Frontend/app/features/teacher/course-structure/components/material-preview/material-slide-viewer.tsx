...
type MaterialSlideViewerProps = {
  material: CourseStructureMaterialDetail;
};

export function MaterialSlideViewer({ material }: MaterialSlideViewerProps) {
  ...
  return (
    <section className="group relative flex aspect-video items-center justify-center overflow-hidden rounded-xl border border-outline-variant/30 bg-surface-container-lowest shadow-[0_4px_20px_rgba(35,39,51,0.04)]">
      <img
        alt={`${material.title} - slide ${activeIndex + 1}`}
        className="h-full w-full bg-surface-container-low object-contain"
